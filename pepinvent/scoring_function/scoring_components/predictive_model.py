...
class PredictiveModel(BaseScoreComponent):
    ...
    def _predict(self, generated_peptides: List[str]) -> np.ndarray:
        ...
        predictions = self._model.predict_proba(x_test) #ANI NOTE: scikit function. The model has estimators which are calibrated by training on data. So if i want to optimise binding affinity, must recompile a new model.
